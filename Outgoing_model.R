...
OM <- function(p1p, p2p, p1m, p2m, bp, bm, l, n0)
{
  ...
  OM_ode <- function(t, vars, params)
  {
    ...
    if ((M < 1-eps) & (M > -1+eps) & (x > eps) & (y > eps) & (z > eps) & (x < L-eps) & (y < L-eps) & (z < L-eps))
    {
      ...
      U[6,] <- c(0, -1, 1, 0)
      U[7,] <- c(0, 0, 0, 0)
      U[14,] <- c(-2*n, x - y, -x, y)/n # NOTE: This is U[14,]
      U[9,] <- c(0, 0, 0, 0)
      U[10,] <- c(0, 0, 0, 0)
      U[11,] <- c(-2*n, x - y, -x, y)/n
      U[12,] <- c(0, -1, 0, 1)
      U[13,] <- c(0, 0, 0, 0)
      U[8,] <- c(2*m, z - y, y, -z)/m # NOTE: This is U[8,]
      U[15,] <- c(0, 0, 0, 0)
      U[16,] <- c(0, 0, 0, 0)
      ...
      return(list(c(0, 0 , 0 ,0))) # Outside boundary conditions, state variables remain fixed
    }
  }
